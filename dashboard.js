(function () {
  ...
  function getStepTimer(match) {
    if (!match) {
      return { stepLabel: "No active step", deadlineLabel: "No active deadline", remaining: "Open" };
    }
    if (match.status === "pending_intro") {
      return { stepLabel: "Intro video", deadlineLabel: "24 hour intro deadline", remaining: formatTimeRemaining(match.introDeadline) };
    }
    if (match.status === "date_planning") {
      return { stepLabel: "Date planning", deadlineLabel: "2 week date deadline", remaining: formatTimeRemaining(match.dateDeadline) };
    }
    return { stepLabel: "Final decision", deadlineLabel: "Decision deadline", remaining: formatTimeRemaining(match.decisionDeadline) };
  }
  ...
  function getMatchStep(match, likes, requests) {
    ...
    if (!match) {
      if (requests.length) {
        return { key: "match_requests", title: "Review match requests", detail: `${requests.length} pending match request${requests.length === 1 ? "" : "s"} need attention before browsing more people.`, ctaLabel: "Open Match Requests", ctaHref: "match-requests.html" };
      }
      if (likes.length) {
        return { key: "review_likes", title: "Review likes", detail: `${likes.length} people already liked this account. Review them and decide whether to send a match request.`, ctaLabel: "Open Likes", ctaHref: "likes.html" };
      }
      return { key: "browse", title: "Browse for a match", detail: `No active match, no pending likes to review, and no match requests waiting. ${user.name} should be in browse until new interest comes in.`, ctaLabel: "Find A Match", ctaHref: "profiles.html" };
    }
    ...
    if (match.status === "pending_intro") {
      return { key: "pending_intro", title: "Matched: intro video step", detail: `A match exists and both people must submit the intro video within 24 hours. Time left: ${formatTimeRemaining(match.introDeadline)}.`, ctaLabel: "Open Match Flow", ctaHref: "match.html" };
    }

    if (match.status === "date_planning") {
      const dateState = AppData.getDatePlanningState(match, user.id);
      return { key: "date_planning", title: dateState.title, detail: `${dateState.detail} Time left: ${formatTimeRemaining(match.dateDeadline)}.`, ctaLabel: "Open Match Flow", ctaHref: "match.html" };
    }

    return { key: "decision_window", title: "Matched: attract or shun step", detail: "The date step is complete. The match is waiting for final outcome decisions.", ctaLabel: "Open Match Flow", ctaHref: "match.html" };
  }
  ...
  function renderFlowPanel(activeMatch, likes, requests) {
    ...
    const steps = [
      {
        ...
        detail: user.onboardingCompleted ? "Completed. This account can move through the app." : "Still incomplete. The user should finish onboarding before discovery.",
      ...
      },
      {
        ...
        detail: activeMatch ? `${nextStep.detail} Current timer: ${stepTimer.remaining}.` : nextStep.detail,
        state: user.onboardingCompleted ? "active" : "",
      },
    ];
    ...
    return `
      <div class="stat-grid">
        <div class="stat-card"><div class="stat-label">Current mode</div><div class="stat-value">${activeMatch ? "Match" : user.onboardingCompleted ? "Browse" : "Onboarding"}</div></div>
        <div class="stat-card"><div class="stat-label">Current step</div><div class="stat-value">${activeMatch ? stepTimer.stepLabel : "Discovery"}</div></div>
        <div class="stat-card"><div class="stat-label">${activeMatch ? stepTimer.deadlineLabel : "Timer"}</div><div class="stat-value">${activeMatch ? stepTimer.remaining : "No active timer"}</div></div>
        <a class="stat-card" href="likes.html"><div class="stat-label">Incoming likes</div><div class="stat-value">${likes.length}</div></a>
        <a class="stat-card" href="match-requests.html"><div class="stat-label">Match requests</div><div class="stat-value">${requests.length}</div></a>
        <div class="stat-card"><div class="stat-label">Availability</div><div class="stat-value">${activeMatch ? "Locked" : "Open"}</div></div>
      </div>
      <div class="timeline">
        ${steps.map((step) => `
          <div class="timeline-row ${step.state}">
            <div><strong>${step.title}</strong><div class="small-copy">${step.detail}</div></div>
    ...
    if (!user.onboardingCompleted) {
      nextEl.innerHTML = `
        <div class="summary-card">
          ...
          <div class="cta-row"><a class="primary-link" href="onboarding.html">Open Onboarding</a></div>
        </div>
      `;
    } else if (!activeMatch) {
